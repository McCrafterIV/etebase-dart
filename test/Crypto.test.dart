...
// import { USER1() } from "./TestConstants";
//
...
void main() async {
  ...
  test('Derive key', () async {
    final derived = await deriveKey(fromBase64(USER1().salt), USER1().password);
    expect(toBase64(derived), equals(USER1().key));
  });

  test('Symmetric encryption', () {
    final key = fromBase64(USER1().key);

    final cryptoManager = CryptoManager(key, 'Col', Constants.CURRENT_VERSION);
    final clearText = fromString('This Is Some Test Cleartext.');
    final cipher = cryptoManager.encrypt(clearText);
    expect(clearText, equals(cryptoManager.decrypt(cipher)));

    final detatched = cryptoManager.encryptDetached(clearText);
    expect(clearText,
        equals(cryptoManager.decryptDetached(detatched[0], detatched[1])));

    var derived = cryptoManager.deriveSubkey(Uint8List(32));
    expect(derived,
        equals(fromBase64('4w-VCSTETv26JjVlVlD2VaACcb6aQSD2JbF-e89xnaA')));

    derived = cryptoManager.calculateMac(Uint8List(32));
    expect(derived,
        equals(fromBase64('bz6eMZdAkIuiLUuFDiVwuH3IFs4hYkRfhzang_JzHr8')));

    derived = cryptoManager.calculateMac(Uint8List(32), false);
    expect(derived,
        equals(fromBase64('iesNaoppHa4s0V7QNpkxzgqUnsr6XD-T-BIYM2RuFcM')));

    final cryptoMac = cryptoManager.getCryptoMac();
    cryptoMac.update(Uint8List(4));
    expect(cryptoMac.finalize(),
        equals(fromBase64('y5nYZ75gDUna4bnaAHobXUlgQoTKOnueNW_KCYxcAg4')));
  });
  ...
  test('Deterministic encryption', () {
    ...
    final cryptoManager = CryptoManager(key, 'Col', Constants.CURRENT_VERSION);
    final clearText = fromString('This Is Some Test Cleartext.');
    final cipher = cryptoManager.deterministicEncrypt(clearText);
    expect(clearText, equals(cryptoManager.deterministicDecrypt(cipher)));

    final cipher2 = cryptoManager.deterministicEncrypt(clearText);
    expect(cipher, equals(cipher2));
  });

  test('Deterministic encryption pre-calculated values', () {
    final key = fromBase64(USER1().key);
    final preCalc = [
    ...
    for (var i = 0; i < blocksize * 2; i++) {
      ...
      buf.fillRange(0, buf.length, 60);
      final cipher = cryptoManager.deterministicEncrypt(buf);
      expect(toBase64(cipher), equals(preCalc[1]));
    }
  });
  ...
  test('Crypto mac', () {
    ...
    var cryptoMac = CryptoMac(null);
    cryptoMac.update(Uint8List(4));
    cryptoMac.updateWithLenPrefix(Uint8List(8));
    expect(cryptoMac.finalize(),
        equals(fromBase64('P-Hpzo86RG6Ps4R1gGXmQrzmdJC2OotqqreKmB8G45A')));

    cryptoMac = CryptoMac(key);
    cryptoMac.update(Uint8List(4));
    cryptoMac.updateWithLenPrefix(Uint8List(8));
    expect(cryptoMac.finalize(),
        equals(fromBase64('rgL6d_XDiBfbzevFdtktc61XB5-PkS1uQ1cj5DgfFc8')));
  });

  test('Pretty fingerprint', () {
    final pubkey = fromBase64(USER1().pubkey);

    final fingerprint = getPrettyFingerprint(pubkey);
    expect(
        fingerprint,
        equals(
